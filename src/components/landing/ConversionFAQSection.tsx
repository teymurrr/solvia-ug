...
const ConversionFAQSection = () => {
  ...
  const items = faq?.items || defaultFAQItems;
  const countryLinks = [
  ...
  // Build FAQ schema from the displayed items
  const faqSchemaItems = items.map((item: any) => ({
  ...
  return (
    <section className="py-16 bg-background">
      <StructuredData data={createFAQSchema(faqSchemaItems)} />
