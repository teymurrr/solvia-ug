export const wizard = {
  welcome: {
    title: 'Bienvenido. Te guiaré paso a paso.',
    subtitle: 'No necesitas conocer el proceso. Solo responde algunas preguntas fáciles.',
    freeGuideTitle: '¡Obtén tu guía de carrera gratuita!',
    freeGuideDescription: 'Completa este breve cuestionario y recibe una guía completa para ayudarte a navegar tu carrera médica en tu país de destino.',
    start: 'Empezar'
  },
  targetCountry: {
    title: '¿Dónde quieres trabajar?',
    subtitle: 'Selecciona tu país de destino'
  },
  studyCountry: {
    title: '¿En qué país obtuviste tu título de medicina?',
    subtitle: 'Solo elige un país',
    placeholder: 'Selecciona tu país...'
  },
  doctorType: {
    title: '¿Qué tipo de profesional médico eres?',
    subtitle: 'Selecciona tu tipo de profesión',
    general: 'Médico General',
    specialist: 'Médico Especialista',
    nurse: 'Enfermero/a',
    dentist: 'Dentista',
    other: 'Otro profesional médico',
    unsure: 'Aún no estoy seguro'
  },
  documents: {
    title: '¿Ya tienes algunos de tus documentos preparados?',
    subtitle: 'Esto nos ayuda a guiarte mejor',
    yes: 'Sí, tengo algunos',
    no: 'No, necesito orientación',
    unsure: 'No tengo idea'
  },
  language: {
    title: '¿Cuál es tu nivel de {language}?',
    subtitle: 'Selecciona tu dominio actual del idioma del país donde quieres trabajar',
    nativeSpeaker: 'Hablante nativo',
    motherTongue: 'Hablante nativo',
    dontKnow: 'No lo sé'
  },
  email: {
    title: '¿Cuál es tu dirección de correo electrónico?',
    subtitle: 'Lo usaremos para enviarte tu plan personalizado',
    placeholder: 'tu.correo@ejemplo.com',
    continue: 'Continuar',
    complete: 'Completar',
    processing: 'Procesando...',
    generatePlan: 'Generar mi plan'
  },
  firstName: {
    title: '¿Cuál es tu nombre?',
    subtitle: 'Queremos conocerte mejor',
    placeholder: 'Nombre',
    continue: 'Continuar',
  },
  lastName: {
    title: '¿Cuál es tu apellido?',
    subtitle: 'Solo un paso más',
    placeholder: 'Apellido',
    continue: 'Continuar',
  },
  password: {
    title: 'Crea una contraseña segura',
    subtitle: '¡Último paso para recibir tu guía de homologación gratuita!',
    placeholder: 'Contraseña (mín. 8 caracteres)',
    confirmPlaceholder: 'Confirmar Contraseña',
    complete: 'Crear Cuenta',
    creating: 'Creando cuenta...',
    mismatch: 'Las contraseñas no coinciden',
    tooShort: 'La contraseña debe tener al menos 8 caracteres',
    privacyConsent: 'Acepto la',
    privacyPolicy: 'Política de Privacidad',
    and: 'y los',
    termsOfService: 'Términos de Servicio',
    privacyError: 'Debes aceptar la Política de Privacidad y los Términos de Servicio'
  },
  summary: {
    title: 'Resumen',
    subtitle: 'Revisa tu información y elige cómo continuar',
    targetCountry: 'País destino',
    studyCountry: 'País de estudio',
    name: 'Nombre',
    professionalType: 'Tipo de profesional',
    languageLevel: 'Nivel de idioma',
    estimatedPrice: 'Precio estimado',
    choosePlan: 'Elegir paquete',
    bookConsultation: 'Reservar consulta gratuita'
  },
  back: 'Atrás',
  countries: {
    germany: 'Alemania',
    austria: 'Austria',
    spain: 'España',
    italy: 'Italia',
    france: 'Francia'
  }
};
