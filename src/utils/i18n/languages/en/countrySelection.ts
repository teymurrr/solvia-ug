export const countrySelection = {
  title: 'Where do you want to work?',
  subtitle: "Choose the country where you want to obtain your professional recognition. You don't need to know the process – we guide you step by step.",
  notSure: 'Not sure which country is right for you?',
  scheduleCall: 'Schedule a free call with us',
  countries: {
    de: {
      name: 'Germany',
      description: 'High demand for doctors and nurses. Clear Approbation pathway.',
    },
    at: {
      name: 'Austria',
      description: 'Strong need for healthcare staff. University recognition system.',
    },
    es: {
      name: 'Spain',
      description: 'Centralized homologación. Popular entry point for Spanish-speaking doctors.',
    },
  },
};
